...
bootfrr()
{
    ...
    for r in rip ripng ospf6 ospf bgp babel isis fabric; do
...
class FRROpenfabric(FrrService):
    """
    The Openfabric service provides IPv4 and IPv6 routing for wired networks. It does
    not build its own configuration file but has hooks for adding to the
    unified frr.conf file.
    """

    name: str = "FRROpenFabric"
    startup: Tuple[str, ...] = ("bash frrboot.sh fabricd",)
    shutdown: Tuple[str, ...] = ("killall fabricd",)
    validate: Tuple[str, ...] = ("pidof fabricd",)
    ipv4_routing: bool = True
    ipv6_routing: bool = True
    ...
    @classmethod
    def generate_frr_config(cls, node: CoreNode) -> str:
        cfg = "router openfabric 1\n"
        cfg += "  net 47.0000.0000.0000.%04x.00\n" % node.id
        cfg += "!\n"
        return cfg

    @classmethod
    def generate_frr_iface_config(cls, node: CoreNode, iface: CoreInterface) -> str:
        cfg = "  ip router openfabric 1\n"
        cfg += "  ipv6 router openfabric 1\n"    
        return cfg
